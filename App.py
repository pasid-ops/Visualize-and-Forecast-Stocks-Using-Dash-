...
import dash_auth  # Import the dash_auth module
...
import os
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'  # Set TensorFlow logging to suppress INFO messages
# Function to calculate LSTM model accuracy
def calculate_lstm_accuracy(model, X_test, y_test, scaler):
    ...
    percentage_difference = np.abs((predicted_prices - actual_prices) / actual_prices) * 100

    # Calculate the mean accuracy
    accuracy = 100 - np.mean(percentage_difference)
    return accuracy
...
def update_prediction(n_clicks, search_query, selected_indicators, n_intervals):
    if n_clicks and search_query:
        ...
        train_lstm_model(lstm_model, X_train, y_train, batch_size=32, epochs=100)

        # Calculate accuracy
        accuracy = calculate_lstm_accuracy(lstm_model, X_test, y_test, scaler)
        ...
        lstm_prediction_signal = "Buy" if lstm_predicted_prices[-1][0] > stock_data['Close'][-1] else "Sell"

        # Print the accuracy and signal in the terminal/cmd
        print(f'LSTM Model Accuracy: {accuracy:.2f}%')
        print(f'{lstm_prediction_signal} {search_query} ')

        # Display the prediction signal and stock name in the app
        output_text = f'{lstm_prediction_signal} {search_query} '
